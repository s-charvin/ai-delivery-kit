...
| All subreqs `merged` | CP-ARCHIVE pause → `archive` (converge status in place) | Creating a second copy of canonical artifacts |
...
| `merged` | any | any | `archive` (update status in place, set `archived`) |
