...
2. Official preview file exists; chat showed its **absolute path** (Flutter: golden PNG from `flutter test --update-goldens`, plus a deterministic GIF for animated units when the host can produce one; if GIF motion cannot be recorded, the index records why and defers runtime verification).
