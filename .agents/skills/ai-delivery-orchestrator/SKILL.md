...
When autonomous execution is needed, use the **external** coordination MCP server (`start_loop`, `intervene_loop`, …). The skill layer keeps spec-segment truth in `status.json`; the engine only writes back execution-segment statuses and immediately re-runs reconcile. **Never import coordination Python from the skill layer** — see [references/coordination-mcp-bridge.md](references/coordination-mcp-bridge.md) and [docs/coordination-repo.md](../../../docs/coordination-repo.md).
