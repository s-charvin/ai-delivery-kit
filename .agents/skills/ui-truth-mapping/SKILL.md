...
- **Real data or intentional emptiness.** If an in-scope data-bound value is absent, null, empty, or not yet available, render the product's real empty/omitted state. Never inject a fake/default record, sample image, placeholder text, or fixture value into production code to make a preview look complete. Test fixtures may provide deterministic bytes only for harness mechanics or evidence-backed assets; never use fixture bytes to stand in for missing in-scope visuals/data or to make a preview appear complete.
...
3. **Assets** — persist supplied Spine/skeletal, Lottie, GIF, or video assets when those exact resource classes are evidenced and bytes exist; a video source asset is never an alternate motion-preview output. If a required resource is unavailable, mark `pending-user` and stop; do not use a poster frame or static substitute without an explicit user disposition, and do not invent the file. Record trigger, playback/loop policy, interruption behavior, off-screen policy, and reduced-motion fallback. If the resource is data-bound and absent, record the real empty/omitted result instead of supplying fixture content.
