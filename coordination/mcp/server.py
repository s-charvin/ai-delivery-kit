...
def _make_skeleton(name: str):
    async def handler(**kwargs):
        trace_id = kwargs.get("__trace_id") or _trace_id_var.get("") or uuid.uuid4().hex
        return {
            "ok": False,
            "data": None,
            "error_code": "E_NOT_IMPLEMENTED",
            "error_message": f"Tool {name} not implemented yet (MVP skeleton)",
            "trace_id": trace_id,
        }
    return handler


def _register_skeletons():
    for tname in _SKELETON_TOOL_NAMES:
        schema = {
            "type": "object",
            "properties": {
                "pipeline_id": {"type": "string"},
        ...
        mcp_tool = mcp_types.Tool(
            name=tname,
            description=f"{tname} (MVP skeleton, not implemented)",
            inputSchema=schema,
        )
        mcp._tools.append(mcp_tool)
        mcp._tool_handlers[tname] = _make_skeleton(tname)
        mcp._tool_schemas[tname] = schema
        mcp._tool_permissions[tname] = {}
