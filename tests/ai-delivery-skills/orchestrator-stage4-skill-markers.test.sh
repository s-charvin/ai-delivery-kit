...
ZH_BASELINE="$(python3 -c 'print("\u65e2\u6709\u57fa\u7ebf")')"
ZH_UNICODE_INCREMENTAL="$(python3 -c 'print("\u589e\u91cf\u6587\u672c\u6309\u5b58\u50a8\u5355\u5143\u5207\u7247")')"
ZH_STAGED_DIFF="$(python3 -c 'print("\u68c0\u67e5 `git diff --cached --name-status`")')"
ZH_UNICODE_SAFE_TEXT="$(python3 -c 'print("Unicode \u5b89\u5168\u7684\u589e\u91cf\u6587\u672c")')"
ZH_STATE_PROPAGATION="$(python3 -c 'print("\u72b6\u6001\u5199\u5165\u6ca1\u6709\u5230\u8fbe\u5b9e\u9645\u6e32\u67d3 owner")')"
ZH_SOURCE_DATA="$(python3 -c 'print("\u6765\u6e90\u6570\u636e\u88ab\u672c\u5730\u9ed8\u8ba4\u503c\u66ff\u6362")')"
ZH_TEST_RESOURCE_LIFECYCLE="$(python3 -c 'print("\u6d4b\u8bd5\u8d44\u6e90\u751f\u547d\u5468\u671f")')"
ZH_DESIGN_EVIDENCE="$(python3 -c 'print("\u4fdd\u6301\u8bbe\u8ba1\u8bc1\u636e\u5c42\u7ea7\u72ec\u7acb")')"
ZH_TOGGLE_LIFECYCLE="$(python3 -c 'print("\u51c6\u5907\u5de5\u4f5c\u4e0e\u5c55\u793a\u5f00\u5173\u8026\u5408")')"
ZH_SHARED_CAPABILITY="$(python3 -c 'print("\u591a\u4e2a\u5165\u53e3\u7684\u80fd\u529b\u5224\u5b9a\u5206\u88c2")')"
...
require "$EN" "classify every failure as an" "EN verification failure classification"
require "$EN" "State writes not reaching the rendered owner" "EN state propagation guard"
require "$EN" "Source data replaced by local defaults" "EN source data fidelity guard"
require "$EN" "Preparation work coupled to a presentation toggle" "EN resource toggle lifecycle guard"
require "$EN" "Capability classification split across entry points" "EN shared capability guard"
require "$EN" "Incremental text sliced by storage units" "EN Unicode-safe incremental text"
require "$EN" 'git diff --cached --name-status' "EN commit scope audit"
require "$ZH" "$ZH_BASELINE" "ZH verification failure classification"
require "$ZH" "$ZH_STATE_PROPAGATION" "ZH state propagation guard"
require "$ZH" "$ZH_SOURCE_DATA" "ZH source data fidelity guard"
require "$ZH" "$ZH_TOGGLE_LIFECYCLE" "ZH resource toggle lifecycle guard"
require "$ZH" "$ZH_SHARED_CAPABILITY" "ZH shared capability guard"
require "$ZH" "$ZH_UNICODE_INCREMENTAL" "ZH Unicode-safe incremental text"
require "$ZH" "$ZH_STAGED_DIFF" "ZH commit scope audit"
...
require "$EN_UI" "Static preview confirmation and motion confirmation/waiver are separate gates" "EN Stage 2 motion gate"
require "$EN_UI" "Keep design evidence layers distinct" "EN design evidence provenance"
require "$ZH_UI" "$ZH_DESIGN_EVIDENCE" "ZH design evidence provenance"
...
require "$EN_FLUTTER_ADAPTER" "Unicode-safe incremental text" "Flutter adapter Unicode boundary"
require "$ZH_FLUTTER_ADAPTER" "$ZH_UNICODE_SAFE_TEXT" "ZH Flutter adapter Unicode boundary"
require "$EN_FLUTTER_ADAPTER" "Test resource lifecycle" "Flutter adapter test resource lifecycle"
require "$ZH_FLUTTER_ADAPTER" "$ZH_TEST_RESOURCE_LIFECYCLE" "ZH Flutter adapter test resource lifecycle"
