...
当生产 `Text` 按可见字符逐步增长（打字机、流式文本、截断或预览）时，必须按 grapheme
集群计数和取前缀，不能按 UTF-16 码元计数。在 Flutter 中优先使用宿主已有的
`characters` API（或其他已经批准的 Unicode 感知工具）；禁止把 `String.length` 与
`substring` 当作字符迭代器。增加 Widget 回归测试，推进经过 emoji 代理对、组合字符和
多码点连接字符的边界，并断言没有框架渲染异常且完成文本与原文一致。这是渲染安全要求，
不是使用视觉占位的理由，也不是新增平行文本组件的理由。

## 测试资源生命周期

测试专用的句柄和控制器应在最后一个断言完成后立即释放，并遵循宿主项目
已经验证过的 binding 生命周期模式。不要假设延迟 teardown 回调一定会在
binding 的测试结束检查之前执行。例如，测试启用 Flutter 语义树后，应在
测试返回前显式释放 `SemanticsHandle`，然后运行整个测试文件，以发现
binding 执行顺序导致的失败。这是测试清理要求，不授权修改生产无障碍行为。
