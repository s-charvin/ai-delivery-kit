...
- 以 `requirement-slice.md` 为种子起草 canonical spec；UI truth 切片中，冻结宿主组件 + 已确认 previews 仍是视觉真值。引用每个 v3 unit/scenario id、证据范围及证据来源；描述运行时行为与验收，不重复 Runtime Coverage 细节。
