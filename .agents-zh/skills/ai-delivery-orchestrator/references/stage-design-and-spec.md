...
- 启用 UI truth 的 unit 在记录的切片 worktree 中由 Stage 2 产出的组件 + 有效 v2 `ui-truth-index.json` 的 profile/state/scenario/coverage 与预览/hash 指针（`ui_truth_mode=figma` 或 `runtime-baseline`）
- API 文档（若有）
- 依赖图
